...
Once the initial conditions are specified, the system evolves under the equations of motion \cref{eq:eoms}. The default option is the same of \texttt{LATTICEEASY}: a second-order symplectic scheme in which the field $\phi(\vec{n})$ and its velocity $\partial_{\tilde\tau} \phi(\vec{n})$ are stored at half-step offsets in time. Because the leapfrog method is standard, we refer the reader to the documentation of \texttt{LATTICEEASY}~\cite{latticeeasy} for details. The code also supports an RK4 integrator and an adaptive RK45 integrator, selectable in \texttt{params.txt}. 
