...
\end{align}
This equation is integrated until each point reaches the reference value $\phi_{\rm ref}$ according to the stopping criterion used by the code. In practice, the implementation uses potential/field-based criteria: for monotonic branches it compares $|\phi|$ to $|\phi_{\rm ref}|$, while for generic potentials it compares $V(\phi)$ to $V(\phi_{\rm ref})$. Accordingly, the automatic choice of $\phi_{\rm ref}$ corresponds to the final-box point minimizing the same criterion (in the generic branch: minimum $V$), rather than explicitly minimizing the full local energy density $K+G+V$. This is expected to be a good approximation when the run ends in a separate-universe, near-attractor regime in which gradients are small and $\dot\phi$ is effectively determined by $\phi$, so constant-$\phi$ slices approximately track constant-energy slices. In non-attractor regimes, this equivalence can break down, and one should test the stability of $\zeta$ against the choice of $\phi_{\rm ref}$ and \texttt{Nend} (or set \texttt{phiref\_manual} explicitly)~\cite{Caravano:2025diq}.
...
All output files are saved in the \texttt{{results/}} directory in \texttt{.dat} format. The public release of the code includes a Python notebook, \texttt{{plot.ipynb}}, which provides examples of how to load and visualize these outputs. Note that not all quantities are computed by default; the generated outputs are controlled via the configuration file \texttt{{parameters.h}}. In \cref{fig}, we show examples of outputs produced by the lattice code in the default setup, which will be discussed in detail below and further in \cref{sec:instructions}. In this section we focus on scalar-sector outputs. Gravitational-wave outputs are documented separately in \cref{sec:GW}.
...
\begin{itemize} 
    ...
    \end{equation}
    which assumes a constant second slow-roll parameter $\eta$ (see \cite{Pi:2022ysn,Caravano:2025diq}). The value of $\eta$ must be set in \texttt{{parameters.h}}. These spectra are evaluated only at the final simulation time, in contrast to the inflaton spectra.
...
firstnumber=55,caption={},numbers=none]
...
\begin{lstlisting}[language=C++, firstnumber=59,caption={},numbers=none]
const double rescale_B = sqrt(V0); // Rescaling factor
\end{lstlisting}
...
\begin{lstlisting}[language=C++, firstnumber=60,caption={},numbers=none]
...
\begin{lstlisting}[language=C++, firstnumber=62,caption={},numbers=none]
const double L = 10.0; // Comoving box size in code units (must be sub-horizon at start)
\end{lstlisting}
The comoving size $L$, together with the number of lattice points $N_{\rm pts}$ (also set in \texttt{{parameters.h}}), determines the infrared and ultraviolet cutoffs of the simulation \cite{Caravano:2021pgc}:\footnote{
...
\begin{lstlisting}[language=C++, firstnumber=63,caption={},numbers=none]
...
\begin{lstlisting}[language=C++, firstnumber=69,caption={},numbers=none]
...
The final time \texttt{Nend} should be long enough for all lattice points to reach a common final constant-field hypersurface $\phi_{\rm ref}$. By default, the simulation sets this surface automatically using the same criterion used to stop the separate-universe evolution (for generic potentials: minimum $V$ at the end of the run; for monotonic branches: extremum of $|\phi|$). This choice approximates a uniform-energy final slice when the dynamics is close to an attractor, but it may be problematic in non-attractor regimes or in models with multiple minima/extended plateaus. In such cases, the user can manually define the hypersurface by uncommenting the following line:
\begin{lstlisting}[language=C++, firstnumber=71,caption={},numbers=none]
// const double phiref_manual = value; // (Optional) manually set the reference $\phi$ value
\end{lstlisting}
...
\end{align}
From these spectra we define the gravitational-wave energy density per logarithmic interval in wavenumber as
\begin{equation}
\label{eq:GW_rho}
...
The computation of SIGWs is controlled via the configuration file \texttt{parameters.h}. The GW calculation during inflation and the optional post-inflationary stage can be enabled through dedicated flags.
...
\begin{itemize}
    \item \texttt{post\_inflation/spectraGW.dat},
    \item \texttt{post\_inflation/spectraGWdot.dat}.
\end{itemize}
The \texttt{post\_inflation/} directory also contains scalar-sector outputs associated with the post-inflationary evolution of the Newtonian potential $\Phi$. The following files may be generated:
\begin{itemize}
    \item \textbf{Background files:} \\\texttt{post\_inflation/sf.dat}, \texttt{post\_inflation/means.dat}, \texttt{post\_inflation/velocity.dat}, and \texttt{post\_inflation/variance.dat}. These files contain, respectively, the simulation time, scale factor $a$, and conformal Hubble rate $\mathcal{H}$, the spatial average $\langle \Phi \rangle$, its time derivative $\langle \Phi' \rangle$, and the variance $\langle \Phi^2 \rangle - \langle \Phi \rangle^2$.
    \item \textbf{Scalar spectra files:} \\ \texttt{post\_inflation/spectra.dat} and \texttt{post\_inflation/spectratimes.dat}. These contain the power spectrum of the Newtonian potential $\Phi$, stored as a concatenated list of spectra evaluated at different times, and the corresponding evaluation times.
\end{itemize}
 From these outputs, one can directly construct the dimensionless tensor power spectrum $\Delta_h^2(k)$ and the gravitational-wave energy density spectrum $\Omega_{\rm GW}(k)$ using \cref{eq:GW_Omega}, together with the background quantities $a(\eta)$ and $\mathcal{H}(\eta)$ provided in \texttt{sf.dat} and \texttt{sf\_post.dat}. Figure~\ref{fig:gw} shows a typical example of the resulting post-inflationary tensor spectra. An explicit example illustrating how to compute $\Omega_{\rm GW}$ from the raw output files and reproduce these plots is provided in the example notebook included with the public release.
