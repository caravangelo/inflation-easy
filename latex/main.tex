...
\begin{itemize}
    ...
    \end{equation}
\end{itemize}
Switching between these two defaults requires two explicit steps. This separation is intentional: the two scenarios correspond to different physics inputs (tabulated numerical potential versus analytic expression), so they use different code branches and different run-time parameter sets.

\textbf{Step 1 (compile-time choice).} In \texttt{parameters.h}, choose which branch to compile:
...
\end{lstlisting}
Because this is a compile-time flag, you must recompile after changing it.

\textbf{Step 2 (run-time parameter set).} Match \texttt{params.txt} to the selected branch. The repository provides two presets: \texttt{params.numerical.txt} and \texttt{params.analytic.txt}. Copy the appropriate one to \texttt{params.txt}:
\begin{itemize}
