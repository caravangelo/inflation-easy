...
void initialize_post_inflation() {
  ...
  DECLARE_INDICES
  #if perform_deltaN
  LOOP f[idx(i,j,k)] = 2.0/3.0 * deltaN[idx(i,j,k)];
  #else
  double fmean = 0.0;
  LOOP fmean += f[idx(i,j,k)];
  fmean = fmean / static_cast<double>(gridsize);
  LOOP f[idx(i,j,k)] = 2.0/3.0 * (f[idx(i,j,k)] - fmean) / (fd[idx(i,j,k)] * pow(a, rescale_s - 1.0) / (ad * pow(a, rescale_s - 2.0)));
  #endif

  fd.resize(N * N * N, 0.0);

  a = 1.0;
  double factor = 1.0 * N; // factor = 1 sets the smallest mode the size of the horizon
  ad = factor * 2.0 * pi / L; //setting ad is delicate, think 
